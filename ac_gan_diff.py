...
if __name__ == '__main__':
    ...
    for epoch in range(1, epochs + 1):
        ...
        noise = np.tile(np.random.uniform(-1, 1, (num_rows, latent_size)),
                        (num_classes, 1))#随机初始化的噪声重复num_cls次

        sampled_labels = np.array([
            [i] * num_rows for i in range(num_classes)
        ]).reshape(-1, 1)#每种标签重复num_rows次
        ...
        ##准备num_rows * num_classes个真实图像的类别标签
        real_labels = y_train[(epoch - 1) * num_rows * num_classes:
                              epoch * num_rows * num_classes]
        indices = np.argsort(real_labels, axis=0)#按类别的大小的索引值取出
        ##准备num_rows * num_classes个真实图像
        real_images = x_train[(epoch - 1) * num_rows * num_classes:
                              epoch * num_rows * num_classes][indices]

        # display generated images, white separator, real images
        img = np.concatenate(
            (generated_images,#400*28*28*1
             np.repeat(np.ones_like(x_train[:1]), num_rows, axis=0),#40*28*28*1 生成例图中的白间隔
             real_images))#400*28*28*1
